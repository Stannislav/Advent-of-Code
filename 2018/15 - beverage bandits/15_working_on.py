...
def find_move(i, j, player, targets):
    ...
    while not q.empty():
        m, n = q.get()
        d = dist[m, n] + 1
        for k, l in adjacent(m, n):
            if map[k, l] != '#' and (k, l) not in players:
                if d < dist[k, l]:
                    dist[k, l] = d
                    q.put((k, l))

    # 2. Select all targets with minimal distance
    min_d = min(dist[t] for t in targets)
    if min_d == float('inf'):  # can't reach any of the targets
        return i, j
    targets = [t for t in targets if dist[t] == min_d]

    # 3. Follow backwards from targets to origin
    origin = []
    q = Queue()
    for t in targets:
        q.put(t)
    while not q.empty():
        ...
        for r in adjacent(*p):
            if dist[r] == 1:
                origin.append(r)
            if dist[r] < d:
                dist[r] = INF  # so that we don't process this point again...
                q.put(r)


    return sorted(origin)[0]
        ...
        for i, j in sorted(players):
            ...
            for (k, l) in enemies_pos:
                for (m, n) in adjacent(k, l):
                    if (m, n) == (i, j):  # Enemy is on adjacent square
                        targets = None
                        break
                    if map[m, n] == '.' and (m, n) not in players:
                        targets.append((m, n))
                else:
                    continue
                break
            ...
            if targets:
                i, j = find_move(i, j, player, targets)
                # distances = []
                # for (k, l) in targets:
