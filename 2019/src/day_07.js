...
// Read data
const data = fs.readFileSync("../input/input_07.txt", "utf-8");
...
getPermutations([5, 6, 7, 8, 9]).forEach(phases => {
    ...
    let pos = 0;
    while (true) {
        amps[pos].receiveInput(signal);
        amps[pos].resume();
        if (amps[pos].outputs.length === 0)
            break;
        else
            signal = amps[pos].outputs.shift();
        pos = (pos + 1) % 5;
    }
